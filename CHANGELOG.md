...
This major release consolidates 35+ architectural and feature enhancements from the intensive development cycle (v1.6.x - v1.9.x), finalizing the library's position as the premier AI-native control protocol for Unity.

### Added
- **AI-Native Tool Discovery**: One-click "Link to Gemini CLI" and schema-powered dynamic discovery (`list_tools`).
- **High-Performance Bridge**: Integrated Python MCP bridge script for direct LLM interaction via Gemini CLI.
- **WebSocket Security Suite**: Implemented protection against CSWSH, DNS Rebinding, and port hijacking.
- **AI UX Dashboard**: Consolidated all controls into a unified "Nexus Unity" Editor window with real-time connectivity status and copyable server URL.
- **Cinematic Automation**: Released demonstration scripts for autonomous scene construction and orchestration.
- **Streaming JSON-RPC**: New high-performance parser using `TextReader` for zero-allocation request processing and minified responses.

### Improved
- **Stability & Self-Healing**: Server now persists through domain reloads and automatically recovers from network-level silent crashes.
- **Deterministic Assets**: `unity_refresh_asset_database` now supports synchronous imports and returns compilation state to prevent agent race conditions.
- **O(1) Dispatcher**: Refactored method routing to use constant-time dictionary lookups.
- **WebSocket Reliability**: Added support for packet fragmentation and message accumulation (> 4KB).

### Fixed
- **Critical Path Traversal**: Hardened `ValidatePath` to prevent unauthorized sibling-directory access.
- **Resource Management**: Fixed `HttpListener` socket leaks and "Port already in use" conflicts.
- **Dispatcher Reliability**: Resolved regression bugs in method prefix routing and argument sanitization.

### Changed
- **Architecture**: Refactored monolithic implementation into modular partial classes (`Scene`, `Asset`, `UI`, `Hierarchy`, etc.).
- **Zero-Dependency Core**: Removed hard dependencies on `EditorCoroutines`, ensuring out-of-the-box compatibility with any Unity project.
