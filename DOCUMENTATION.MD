...
Unity Package Manager requires `MAJOR.MINOR.PATCH` values in `package.json`, so the technical package version uses forms like `1.3.0`. GitHub release tags and titles use the shorter `v1.3` / `1.3` naming style when the patch component is zero.
...
- Tag the release with the matching short GitHub version, for example `v1.3` for package version `1.3.0`.
